...
import { FaLinkedin, FaGithub, FaCar, FaHome, FaInfo, FaUsers } from "react-icons/fa";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const { outsideClickRef } = useOutsideClick<HTMLDivElement>(() => setIsMenuOpen(false));
  return (
    <div className="h-screen relative w-screen bg-neutral-100 overflow-y-auto no-scrollbar overflow-x-hidden">
      ...
      <div className="flex shrink-0 items-center justify-center h-16 bg-white w-screen shadow-lg">
        <div className="xl:max-w-6xl w-full mx-auto h-full flex items-center justify-between   px-4">
          ...
          <Link href="/">
            <Image src="/logo.svg" alt="logo" width={1612} height={96.89} className="h-10 w-fit object-contain" />
          </Link>
      ...
      <div className="max-w-6xl mx-auto flex w-full shrink-0 min-h-[calc(100vh-4rem)]">{children}</div>
      <Footer />
    </div>
  );
}
...
function AnimatedMenuButton({ isClosed, onClick }: { isClosed: boolean; onClick: () => void }) {
  return (
    <motion.button onClick={onClick} className="relative h-6 w-6 flex items-center justify-center">
      <motion.span
        animate={{
